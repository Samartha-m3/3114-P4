...
// w.intersect(0, 0, 0, 1024, 1024, 1024));
    ...
    public void testSkipListInsertion() throws Exception {
        Random rnd = new Random();
        rnd.setSeed(0xCAFEBEEF);
        WorldDB w = new WorldDB(rnd);
        ...
        assertEquals("Node has depth 3, Value (null)\r\n"
    ...
    public void testIsValid() {
        int ws = 1024; 
        WorldDB w = new WorldDB(new Random());

        assertFalse(w.add(null));
        assertFalse(w.add(new Drone(null, 10, 10, 10, 1, 1, 1, "type", 1)));
        assertFalse(w.add(new Drone("", 10, 10, 10, 1, 1, 1, "type", 1)));
        assertFalse(w.add(new Drone("Name", 1, ws, 1, 1, 1, 1, "type", 1)));
        assertFalse(w.add(new Drone("Name", 1, 1, 1, ws + 1, 1, 1, "type", 1)));
        assertFalse(w.add(new Drone("Name", 1, 1, 1, ws, 1, 1, "type", 1)));
        assertTrue(w.add(new Drone("Name", 0, 0, 0, ws, ws, ws, "type", 1)));
    }
    ...
    public void testSkipListDelete() throws Exception {
        ...
        assertTrue(w.add(new Rocket("Enterprise", 0, 100, 20, 10, 50, 50, 5000,
            99.29)));
        assertNull(w.delete("NoSuchElement"));

        assertEquals("Airplane Air1 0 10 1 20 2 30 USAir 717 4", w.delete(
            "Air1"));
        ...
        assertEquals("Drone Air2 100 1010 101 924 2 900 Droners 3", w.delete(
            "Air2"));
        assertEquals("Node has depth 3, Value (null)\r\n"
            + "Node has depth 2, Value (Balloon B1 10 11 11 21 12 31 hot_air 15)\r\n"
            + "Node has depth 1, Value (Rocket Enterprise 0 100 20 10 50 50 5000 99.29)\r\n"
    ...
    public void testRangePrint() {
        ...
        assertEquals("Found these records in the range Air2 to Enterprise\r\n"
            + "Drone Air2 100 1010 101 924 2 900 Droners 3\r\n"
            + "Balloon B1 10 11 11 21 12 31 hot_air 15\r\n"
            + "Rocket Enterprise 0 100 20 10 50 50 5000 99.29\r\n", w
                .rangeprint("Air2", "Enterprise"));

        assertNotNull(w.rangeprint("b", "z"));
        assertEquals("Found these records in the range b to z\r\n"
            + "Bird pterodactyl 0 100 20 10 50 50 Dinosaur 1\n", w.rangeprint(
                "b", "z"));

        assertNotNull(w.rangeprint("Air2", "Air2"));
        assertEquals("Found these records in the range Air2 to Air2\r\n"
            + "Drone Air2 100 1010 101 924 2 900 Droners 3\n", w.rangeprint(
                "Air2", "Air2"));

        assertNotNull(w.rangeprint("Air1", "zzz"));
        assertEquals("Found these records in the range Air1 to zzz\r\n"
            + "Airplane Air1 0 10 1 20 2 30 USAir 717 4\r\n"
            + "Drone Air2 100 1010 101 924 2 900 Droners 3\r\n"
            + "Balloon B1 10 11 11 21 12 31 hot_air 15\r\n"
            + "Rocket Enterprise 0 100 20 10 50 50 5000 99.29\r\n"
    ...
    public void testBadRangePrint() {
        Random rnd = new Random();
        rnd.setSeed(0xCAFEBEEF);
        WorldDB w = new WorldDB(rnd);

        assertNull(w.rangeprint(null, null));
    ...
    public void testPrint() {
        ...
        assertEquals("Drone Air2 100 1010 101 924 2 900 Droners 3", w.print("Air2"));
        assertNull(w.print("NoSuchElement"));
